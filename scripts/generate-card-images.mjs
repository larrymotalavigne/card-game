...
/**
 * Generate pixel art card images using Hugging Face Inference API.
 *
 * Usage:
 *   HF_TOKEN=hf_xxx node scripts/generate-card-images.mjs
 *   HF_TOKEN=hf_xxx node scripts/generate-card-images.mjs --domain IT
 *   HF_TOKEN=hf_xxx node scripts/generate-card-images.mjs --only it-001,it-002
 *   HF_TOKEN=hf_xxx node scripts/generate-card-images.mjs --dry-run
 *   HF_TOKEN=hf_xxx node scripts/generate-card-images.mjs --force
 */
...
const MODEL = 'black-forest-labs/FLUX.1-schnell'; // Fast, free, good quality
...
const RARITY_PROMPT = {
  ...
  'Peu commune': 'detailed design',
  Rare: 'elaborate design with glowing accents',
  Légendaire: 'epic, dramatic lighting, golden aura, legendary',
};

const TYPE_PROMPT = {
  Métier: 'character portrait, person at work',
  Outil: 'iconic object, item, equipment',
...
function parseCardsFromFile(filePath) {
  ...
  // Match each card object literal (handles escaped quotes inside strings)
  ...
  while ((match = cardBlockRegex.exec(content)) !== null) {
    ...
    const getQuoted = (key) => {
      // Handle escaped single quotes inside strings: 'l\'avion'
      const m = block.match(new RegExp(`${key}:\\s*'((?:[^'\\\\]|\\\\.)*)'`));
      return m ? m[1].replace(/\\'/g, "'") : '';
    };

    // Map enum member names to French values
    const domainMap = {
...
function buildPrompt(card) {
  ...
  // Translate card name to English context using the domain + flavor text
  const flavorHint = card.flavorText
    ? `, inspired by: ${card.flavorText.replace(/[«»"]/g, '')}`
    : '';

  const abilityHint = card.ability || card.effect || '';
  const contextHint = abilityHint ? `, context: ${abilityHint}` : '';

  return [
    `16-bit pixel art illustration, retro RPG video game style, visible pixels, limited color palette.`,
    `Clean composition, centered subject, no text, no letters, no words, no UI elements.`,
    `Solid ${domainInfo.color} tinted background.`,
    `${rarityHint}.`,
    `Subject: ${card.name} — ${typeHint}, ${domainInfo.en} themed${flavorHint}.`,
  ].join(' ');
}
...
async function main() {
  ...
  if (!force) {
    toGenerate = cards.filter(c => {
      ...
      if (existsSync(pngPath)) {
        ...
        return stat.size < 1000; // Skip if >1KB (valid image)
      }
      return true;
    });
  ...
  console.log(`\nWill generate ${toGenerate.length} images using model: ${MODEL}`);
  ...
  if (failures.length > 0) {
    const failedFile = join(__dirname, 'failed-cards.txt');
    writeFileSync(failedFile, failures.join('\n') + '\n');
    console.log(`\nFailed IDs written to: ${failedFile}`);
    console.log(`Re-run with: HF_TOKEN=... node scripts/generate-card-images.mjs --only ${failures.join(',')}`);
  }
}
